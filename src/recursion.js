...
var rMap = function(array, callback) {
	if (array.length === 1) return timesTwo(array.shift());
	return [timesTwo(array.shift())].concat(rMap(array));
};
function timesTwo(n) {
	return n * 2;
}
...
var alternateSign = function(array) {
	...
	if (array.length === 2)
		return array[0] ? Math.abs(array[1]) : Math.abs(array[1]) * -1;
...
var numToText = function(str) {
	...
	if (strArr.length === 1)
		return !isNaN(Number(strArr[0]))
			? numNameRef[Number(strArr[0])]
			: strArr[0];
	var strRet = strArr.slice(1).join(' ');
	return !isNaN(Number(strArr[0]))
		? numNameRef[Number(strArr[0])] + ' ' + numToText(strRet)
		: strArr[0] + ' ' + numToText(strRet);
};
